//#region src/types/class/object.d.ts
declare class EntryObject {
  name: string;
}
//#endregion
//#region src/types/class/entity.d.ts
declare class EntityObject {
  parent: EntryObject;
}
//#endregion
//#region src/types/util/static.d.ts
declare const STATIC: {
  ...
  BOX_MODEL: 10;
  THREAD_MODEL: 11;
  DRAG_INSTANCE: 12; /* block state */
  BLOCK_STATIC: 0;
  BLOCK_MOVE: 1;
  BLOCK_FOLLOW: 2; /* execute return state */
  RETURN: 0;
  CONTINUE: 1;
  BREAK: 2;
  PASS: 3; //if command type number > 500
  ...
  COMMAND_TYPES_ALWAYS: {
    ...
    dataTableRemoveSource: 1302; //TODO commands development
  };
...
//#endregion
//#region src/types/playground/scope.d.ts
declare class Scope<ParamsKey extends string> {
  ...
  getParam(this: this, index: number): unknown; // 클래스 레벨에서 한 번만 생성
  ...
  getValues<const T extends ParamsKey[]>(this: this, keys: T, scope?: this): { [K in keyof T]: unknown };
  /**
 ...
 */
declare function updateCategory<const Blocks extends string[]>(category: string, blocks: Blocks, callback: (addBlock: ReturnType<typeof getAddBlock<Blocks[number]>>) => void, options: {
  name?: string;
  background?: string;
  backgroundOn?: string;
...
declare const getAddBlock: <BlockName extends string>(categoryName: string) => <ParamsKey extends string>(blockName: LiteralUnion<BlockName>, template: string, colors: {
  color: string;
  outerline: string;
}, param: {
